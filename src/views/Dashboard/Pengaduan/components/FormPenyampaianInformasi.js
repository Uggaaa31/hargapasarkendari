// src/views/Dashboard/Pengaduan/components/FormPenyampaianInformasi.js
...
function FormPenyampaianInformasi({ onClose }) {
  const toast = useToast(); // <<< === Inisialisasi useToast ===

  const [formData, setFormData] = useState({
    // ==== IDENTITAS PENGADU (Nama field disesuaikan dengan snake_case Laravel) ====
    nama: "",
    email: "",
    ...
    uraian_informasi: "", // <<< === 'uraianInformasi' menjadi 'uraian_informasi' ===
  });
  ...
  // KODE INI HAMPIR SAMA DENGAN FormPengaduan.js, SUDAH SAYA PERIKSA.
  // URL API PUBLIKNYA SUDAH BENAR SESUAI PEMBAHASAN SEBELUMNYA.
  // ==============================================================================================
  ...
  useEffect(() => {
    const fetchProvinces = async () => {
      ...
      try {
        ...
        setGeneralError("Gagal memuat daftar provinsi dari API publik. Silakan coba lagi."); // Menggunakan generalError
      } finally {
  ...
  useEffect(() => {
    if (formData.provinsi) { // Menggunakan 'provinsi'
      ...
      const fetchCities = async () => {
        try {
          ...
          setGeneralError("Gagal memuat daftar kota/kabupaten dari API publik. Silakan coba lagi."); // Menggunakan generalError
        } finally {
    ...
    setFormData(prev => ({ ...prev, kabupaten: null })); // Menggunakan 'kabupaten'
  }, [formData.provinsi]); // Menggunakan 'provinsi'
  ...
  const handleSubmit = async (e) => { // <<< === Jadikan async ===
    ...
    const dataToSend = {
      ...
      uraian_informasi: formData.uraian_informasi, // Mengirim 'uraian_informasi'
    };
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/api/informasi", { // <<< === URL API Laravel untuk Informasi ===
        method: "POST",
        headers: {
      ...
      if (response.ok) { // Status 2xx
        toast({
          title: "Informasi Berhasil Disampaikan!",
          description: result.message || "Informasi Anda telah berhasil dikirim.",
          status: "success",
          duration: 5000,
          isClosable: true,
        });
        ...
        let errorMessage = result.message || "Gagal menyampaikan informasi. Silakan coba lagi.";
        if (result.errors) {
  ...
  return (
    <Box as="form" onSubmit={handleSubmit} p={{ base: "10px", md: "20px" }}>
      ...
      <SimpleGrid columns={{ base: 1, md: 2 }} spacing={{ base: "15px", md: "20px" }}>
        ...
        <FormControl id="jenis_kelamin"> {/* <<< === Sesuaikan nama prop === */}
          ...
          <Select
            ...
            onChange={(selectedOption) => handleSelectChange(selectedOption, {name: 'jenis_kelamin'})} // <<< === Sesuaikan name ===
      ...
      <Heading size="md" mt="40px" mb="20px">II. INFORMASI YANG DISAMPAIKAN</Heading>
      <FormControl id="uraian_informasi" isRequired> {/* <<< === Sesuaikan nama prop === */}
        <FormLabel fontSize="sm">Uraian Informasi</FormLabel>
        <Textarea name="uraian_informasi" value={formData.uraian_informasi} onChange={handleInputChange} rows={5} size="md" />
      </FormControl>
...
export default FormPenyampaianInformasi;